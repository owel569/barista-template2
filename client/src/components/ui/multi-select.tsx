...
export function MultiSelect({
  ...
  const filteredOptions = React.useMemo(() => {
    ...
    return validatedOptions.filter((option) =>
      option.label.toLowerCase().includes(searchTerm);}, [validatedOptions, search, searchable])

  // Validation des options sélectionnées
  const validatedSelected = React.useMemo(() => {
    const validValues = new Set(validatedOptions.map(opt => opt.value);return selected.filter(value => validValues.has(value);}, [selected, validatedOptions])
