...
import { SalesPrediction, AIInsight } from @/types/admin;""""
...
const AdvancedAnalyticsDashboard: React.FC = () => {"""
  ...
  const [selectedView, setSelectedView] = useState<unknown><unknown><unknown>("overview);
  const [isExporting, setIsExporting] = useState<unknown><unknown><unknown>(false);
  const [salesPrediction, setSalesPrediction] = useState<unknown><unknown><unknown><SalesPrediction | null>(null);"
  const [aiInsights, setAiInsights] = useState<unknown><unknown><unknown><AIInsight[]>([]);"""
""
...
""
  const fetchSalesPrediction = async () => {"""
    try {""""
      const response = await apiClient.get<{ data: SalesPrediction }>("/analytics/sales-prediction);
  ...
  ""
        ...
        message: Impossible de charger les prédictions de ventes""
      });"
    }""
  };"""
""
  const fetchAIInsights = async () => {"""
    try {""
      ...
      toast({"""
        ...
        title: Erreur de chargement"","
  ""
        message: ""Impossible de charger les insights IA
      });
    }
  };
  ...
  const handleExport: unknown = async () => {
    ...
    try {''
      ...
      toast({"""
        title: Export réussi","
  """
        message: "Les données analytics ont été exportées avec succès"""
      });""
    } catch (error: unknown: unknown: unknown: unknown: unknown: unknown) {"""
      toast({""""
        title: Erreur d"export,"""
        description: "Une erreur est survenue lors de lexport"","
  ""
        variant: ""destructive
      });
    } finally {
...
"""
  return (""
    <div className=""space-y-6"></div>"""
      ...
      {/* KPI temps réel */}"
      <RealTimeKPIs data={analyticsData?.realTimeKPIs as any} ></RealTimeKPIs>""
"""
      {/* Panneau dinsights IA */}""
...
"""
      ...
      <Tabs value=""{selectedView"} onValueChange={setSelectedView""}></Tabs>"''""''"
        ...
        <TabsContent value=""overview" className=""space-y-6"></TabsContent>"""
          <RevenueView timeRange={"timeRange} ></RevenueView>"""
        </TabsContent>""
        """
        <TabsContent value="revenue className=""space-y-6"></TabsContent>"""
          <RevenueView timeRange={timeRange"} ></RevenueView>"""
        </TabsContent>""
        """
        <TabsContent value="customers"" className="space-y-6></TabsContent>"
          <CustomerBehaviorView data={analyticsData?.customerBehavior as any} ></CustomerBehaviorView>"""
        </TabsContent>""
        """
        <TabsContent value=predictions" className=space-y-6""></TabsContent>""
          <PredictiveAnalyticsView data={""analyticsData} ></PredictiveAnalyticsView>
        </TabsContent>
      </Tabs>'"
    </div>"''"
  );""'''"
