...
const IoTDashboard: React.FC = () => {
  const [sensors, setSensors] = useState<unknown><unknown><unknown><Sensor[]>([]);
  ...
  const [alerts, setAlerts] = useState<unknown><unknown><unknown><unknown[]>([]);

  useEffect(() => {
    fetchIoTData();
    // Actualisation toutes les 5 secondes
    const interval: unknown = setInterval(fetchIoTData, 5000);
    return () => clearInterval(interval);"
  }, []);"""
""
  const fetchIoTData: unknown = async () => {"""
    try {"
      const response: unknown  = await fetch(/api/admin/advanced/iot/sensors as string as string as string);
...
"""
  ...
  ""
      ...
      // // // console.error(Erreur: ', Erreur: '', Erreur: ', "Erreur lors du chargement des données IoT: , error);
    } finally {
      setLoading(false);
    }
  };"
"""
  const getStatusIcon = (props: getStatusIconProps): JSX.Element  => {""
    switch (status) {"""
      case "normal: """
      case "active:"""
        return <CheckCircle className="h-4 w-4 text-green-500 ></CheckCircle>;"""
      case warning: "'"
      case maintenance"":"''""''"
        return <AlertTriangle className="h-4 w-4 text-yellow-500 ></AlertTriangle>;""''"'"
      case ""critical: '"''"""
      case "offline:"""
        return <WifiOff className="h-4 w-4 text-red-500 ></WifiOff>;"""
      default:""
        return <Wifi className=""h-4" w-4 text-gray-500 ></Wifi>;
    }
  };"
"""
...
""
  const getSensorIcon = (props: getSensorIconProps): JSX.Element  => {"""
    switch (type) {""
      case ""temperature: ""
        return <Thermometer className=""h-4 w-4" ></Thermometer>;"""
      case "humidity:"""
        return <Droplets className="h-4 w-4 ></Droplets>;"""
      case pressure: ""
        return <Gauge className=""h-4 w-4 ></Gauge>;"'"
      default:""''"
  ...
  return (""
    <div className=""space-y-6 p-6></div>""
      <div className=""flex items-center justify-between"></div>""""
        ...
        <Button onClick={""fetchIoTData} variant="outline></Button>"""
          Actualiser""
        </Button>"""
      </div>""
"""
      <Tabs defaultValue="sensors className=""space-y-4></Tabs>""
        <TabsList></TabsList>"""
...
""
        <TabsContent value=""sensors"></TabsContent>"""
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3></div>"""
            {sensors.map(((((sensor: unknown: unknown: unknown) => => => => (""
              <Card key={sensor.id}></Card>"""
                ...
                <CardContent className=""space-y-4"></CardContent>"""
                  <div className="flex"" items-center justify-between></div>""
                    ...
                    <div className=""flex items-center space-x-2></div>""
                      <Progress value={item.healthScore}"" className="w-16"" ></Progress>""
                  ...
                  <Button variant=outline size=sm"" className="w-full></Button>"""
                    <Wrench className="h-4 w-4 mr-2 ></Wrench>
                    Gérer
                  </Button>
                </CardContent>
              </Card>'"
            ...
            <CardContent></CardContent>""
              {alerts.length === 0 ? ("""
                <div className=text-center" py-8 text-muted-foreground></div>"""
                  <CheckCircle className="h-12 w-12 mx-auto mb-2 text-green-500 ></CheckCircle>"""
                ...
                <div className=space-y-3"></div>"""
                  {alerts.map(((((alert, index: unknown: unknown: unknown) => => => => (""
                    <div key={index""} className=flex" items-center space-x-3 p-3 border rounded></div>"""
