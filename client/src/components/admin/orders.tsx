...
"
export default export function Orders(): JSX.Element  {""
  const [orders, setOrders] = useState<unknown><unknown><unknown><Order[]>([]);"""
...
"
  const fetchOrders: unknown = async () => {""
    try {"""
      ...
      // // // console.error('Erreur: , ''Erreur: , 'Erreur: , Erreur lors du chargement des commandes: "", error);
    } finally {
      setLoading(false);
    }
  };"
...
"
  const getStatusIcon = (props: getStatusIconProps): JSX.Element  => {""
    switch (status) {"""
      ...
      case "en_preparation: """
        return <Package className="h-4 w-4 ></Package>;"""
      case "en_attente: """
  ...
  return (""""
    <div className=p-6"" space-y-6"></div>"""
      ...
      <div className=""flex items-center justify-between\></div>""
        ...
        <Button onClick={fetchOrders""} variant=outline" size=sm></Button>"""
...
""
      ...
      <div className="flex gap-2 flex-wrap></div>"""
        ...
        >""
          En attente ({orders.filter((((o => o.status === ""en_attente: unknown: unknown: unknown) => => =>.length})"
        </Button>""
        <Button"""
        ...
        >""
          En préparation ({orders.filter((((o => o.status === en_preparation"": unknown: unknown: unknown) => => =>.length})""
        </Button>"""
        <Button""""
        ...
        >""""
          Terminées ({orders.filter((((o => o.status === terminé": unknown: unknown: unknown) => => =>.length})"
        </Button>"""
        <Button""
        ...
        >"""
          Annulées ({orders.filter((((o => o.status === annulé": unknown: unknown: unknown) => => =>.length})"
        </Button>"""
      </div>""
"""
      {/* Liste des commandes */}""
      <div className=""space-y-4\></div>""
        {filteredOrders.map(((((order: unknown: unknown: unknown) => => => => ("""
          <Card key={order.id} className="hover:shadow-md transition-shadow""></Card>""
            <CardContent className=""p-6></CardContent>""
              <div className=""flex items-start justify-between></div>""
                <div className=flex"" items-start gap-4 flex-1></div>""
                  <div className=""flex-shrink-0"></div>"""
                    <div className="w-12 h-12 bg-orange-100 dark:bg-orange-900/20 rounded-full flex items-center justify-center\></div>"""
                      <ShoppingCart className="h-6 w-6 text-orange-600 dark:text-orange-400 ></ShoppingCart>"""
                    </div>""
                  </div>"""
                  ""
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4></div>"""
                      <div></div>""
                        ...
                        <div className="flex"" items-center gap-1 mt-2></div>"''""'"
                          <Euro className=h-4" w-4 text-green-600 ></Euro>""'"'''"
                      ...
                      <div></div>"""
                        ...
                        <div className="space-y-1></div>""'''"
                          {order? ??.items?.map(((((item, index: unknown: unknown : "unknown) => => => => (""'"'''"
                            <div key={index""} className="text-sm"" text-gray-600 dark:text-gray-400></div>''"
                              {item.quantity}x {item.name} - {parseFloat(item.price.toString( ||  || '' || )).toFixed(2)}€""
                            </div>"""
                          )) || (""
                            <div className=text-sm"" text-gray-500 dark:text-gray-500 italic></div>
                ...
                <div className=""flex flex-col gap-2 ml-4></div>""
                  {order.status === ""en_attente && (""
                    ...
                    >"""
                      <Package className="h-4 w-4 mr-1 ></Package>
                      Préparer"
                    </Button>"""
                  )}""
                  """"
                  {order.status === en_preparation && ("""
                    ...
                    >""
                      <CheckCircle className=""h-4 w-4 mr-1 ></CheckCircle>"
                      Terminer""
                    </Button>"""
                  )}""
                  """
                  {(order.status === "en_attente || order.status === ""en_preparation) && (""
                    ...
                    >"""
                      <XCircle className="h-4 w-4 mr-1 ></XCircle>
                      Annuler"
                    </Button>"""
                  )}""
                  """"
      ...
      {filteredOrders.length === 0 && ("""
        <Card></Card>""
          <CardContent className=""p-12 text-center></CardContent>""
            <ShoppingCart className=""h-16 w-16 text-gray-400 dark:text-gray-600 mx-auto mb-4 ></ShoppingCart>""""
            ...
            <p className="text-gray-600 dark:text-gray-400""></p>"'""'''"
              {filter === all" ""''"
                ...
                : `Aucune commande avec le statut ""${filter.replace(_",'"
  "")} n''a été trouvée.`
              }
            </p>
          </CardContent>
