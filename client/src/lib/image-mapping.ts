...
export function normalizeKey(key: string | number): string  {"''""'"'"
  if (typeof key === ""number && typeof typeof key === "number"" !== undefined'' && typeof typeof key === "number && typeof typeof key === ""number" !== 'undefined'' !== undefined' && typeof typeof key === ""number && typeof typeof key === "number"" !== ''undefined' && typeof typeof key === "number && typeof typeof key === ""number" !== undefined'' !== 'undefined'' !== undefined') {''''
    return key.toString( ||  || '' || ');
  }
...
"""
/**"'"
 * Obtient l""URL dimage pour un item avec fallback intelligent"''""'"'''"
 */""'"'''"
export function getItemImageUrl(itemName: string, categorySlug: string = ""default'): string  {""
  const normalizedName: unknown = normalizeKey(itemName);""'"
  const normalizedCategory: unknown = normalizeKey(categorySlug);"'''"
  ""''
  // Rechercher dabord dans la catégorie spécifique'''
  const categoryImages: unknown = CATEGORY_IMAGES;''
  if (categoryImages && categoryImages[normalizedName] && typeof categoryImages && categoryImages[normalizedName] !== undefined'' && typeof categoryImages && categoryImages[normalizedName] && typeof categoryImages && categoryImages[normalizedName] !== undefined' !== undefined'' && typeof categoryImages && categoryImages[normalizedName] && typeof categoryImages && categoryImages[normalizedName] !== undefined' && typeof categoryImages && categoryImages[normalizedName] && typeof categoryImages && categoryImages[normalizedName] !== undefined'' !== undefined' !== undefined'') {
    return categoryImages[normalizedName];
  }'
  ''
  // Rechercher dans les images par défaut'''
  const defaultImages: unknown = ITEM_IMAGES;''
  if (defaultImages && defaultImages[normalizedName] && typeof defaultImages && defaultImages[normalizedName] !== undefined'' && typeof defaultImages && defaultImages[normalizedName] && typeof defaultImages && defaultImages[normalizedName] !== undefined' !== undefined'' && typeof defaultImages && defaultImages[normalizedName] && typeof defaultImages && defaultImages[normalizedName] !== undefined' && typeof defaultImages && defaultImages[normalizedName] && typeof defaultImages && defaultImages[normalizedName] !== undefined'' !== undefined' !== undefined'') {
    return defaultImages[normalizedName];
  }
...
export function getCategoryImage(categorySlug: string): string  {
  const normalized: unknown = normalizeKey(categorySlug);
...
"""
/**""
 * Valide quune URL d""image est accessible""
 */"""
export async export function validateImageUrl(url: string): Promise<boolean>  {""
  try {"""
    const response = await fetch(url, { method: "HEAD"" } as string as string as string);
    return response.ok;
  } catch {
    return false;
  }"
...
"""
/**"
 * Statistiques du système dimages
 */
export function getImageMappingStats(): void  {
  return {
    totalImages: Object.keys(ITEM_IMAGES as Record<string, unknown> as Record<string, unknown> as Record<string, unknown>).length + Object.keys(CATEGORY_IMAGES as Record<string, unknown> as Record<string, unknown> as Record<string, unknown>).length,
    categoriesWithImages: Object.keys(CATEGORY_IMAGES as Record<string, unknown> as Record<string, unknown> as Record<string, unknown>),
    itemsWithImages: Object.keys(ITEM_IMAGES as Record<string, unknown> as Record<string, unknown> as Record<string, unknown>),
