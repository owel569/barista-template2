...
/**
 * Script de correction finale - Traite les derniers problèmes de guillemets
 */

import fs from 'fs';
import { glob } from 'glob';

// Fonction pour corriger les guillemets multiples
function fixMultipleQuotes(content: string): string {
  // Corriger les guillemets doubles multiples
  content = content.replace(/"""/g, '"');
  content = content.replace(/""""/g, '"');
  content = content.replace(/"""""/g, '"');
  content = content.replace(/""""""/g, '"');
  
  // Corriger les guillemets simples multiples
  content = content.replace(/'''/g, "'");
  content = content.replace(/''''/g, "'");
  content = content.replace(/'''''/g, "'");
  content = content.replace(/''''''/g, "'");
  
  // Corriger les backticks multiples
  content = content.replace(/```/g, '`');
  content = content.replace(/````/g, '`');
  content = content.replace(/`````/g, '`');
  content = content.replace(/``````/g, '`');
...
// Fonction pour corriger les chaînes mal formatées
function fixMalformedStrings(content: string): string {
...
function fixFile(filePath: string): boolean {
  try {
    ...
    const newContent1 = fixMultipleQuotes(content);
    if (newContent1 !== content) {
      content = newContent1;
      hasChanges = true;
    }
    
    const newContent2 = fixMalformedStrings(content);
    if (newContent2 !== content) {
      content = newContent2;
      hasChanges = true;
    }
    
    const newContent3 = fixConditionalExpressions(content);
    if (newContent3 !== content) {
      content = newContent3;
      hasChanges = true;
    }
    
    const newContent4 = fixMethodCalls(content);
    if (newContent4 !== content) {
      content = newContent4;
      hasChanges = true;
    }
    
    const newContent5 = fixObjectLiterals(content);
