...
export class AIAutomationModule {
  ...
  private static async generateChatResponse(message: string, context?: string) {
    ...
    if (lowerMessage.includes('''menu'') || lowerMessage.includes('''carte')) {''
      ...
      return {
        text: `Voici notre menu: ${menuItems}  Que souhaitez-vous commander ?`,''
    ...
    if (lowerMessage.includes('''horaires'') || lowerMessage.includes('''ouvert')) {''
      ...
      return {''
        text: `Nos horaires d'''ouverture: ${hours}`,
        actions: [],
        confidence
      };
    }
    ...
    return {''
      text: 'Bonjour ! Je suis votre assistant virtuel du Café Barista. Je peux vous aider avec: • Notre menu et prix • Les réservations • Nos horaires • Le WiFi et services  Que puis-je faire pour vous ?''',
      actions: [''
