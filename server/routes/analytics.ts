...
interface MockStorage {
  getDailyRevenue: (date: string) => Promise<{ total: number; orderCount: number }>;
  getActiveOrdersCount: () => Promise<number>;
  getOccupiedTablesCount: () => Promise<number>;
  getTotalTablesCount: () => Promise<number>;
  getPendingReservationsCount: () => Promise<number>;
  getPopularDishes: (days: number) => Promise<Array<{ name: string; count: number }>>;
  getLowStockItems: () => Promise<Array<{ item: string; level: number; threshold: number }>>;
}
...
interface Predictions {
  ...
  stockAlerts: Array<{ item: string; level: number; threshold: number }>;
}
...
router.get('''/customer-analytics'', asyncHandler(async (req, res) => {
  try {
    ...
    const analytics = {'
      ...
      topChannels: ['''Email'', '''SMS', '''Réseaux sociaux''],
      campaignROI: 2.5
    };
    ...
    res.json({
      ...
      marketing: {
        ...
        bestChannels: (analytics as AnalyticsData).bestChannels || ['''Email'', '''SMS'],
        campaignROI: (analytics as AnalyticsData).campaignROI || 2.5
      }
    });
  } catch (error) {''
