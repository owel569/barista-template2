...
interface AIReservation {
  ...
  createdAt: string;
  aiGenerated: boolean;
  [key: string]: unknown; // Pour les propriétés dynamiques
}
...
interface AutomationSuggestions {
  menu: {
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (availability.available) {
        ...
        const alternatives = await generateAlternativeSlots(date, time || '''12:00', guests);
        res.json({
          success: false,''
...
async function checkTableAvailability(date: string, time: string, guests: number): Promise<TableAvailability> {
...
async function createAIReservation(data: Record<string, unknown>): Promise<AIReservation> {
...
async function generateAlternativeSlots(date: string, time: string | undefined, guests: number): Promise<AlternativeSlot[]> {''
  const defaultTime = time || '12:00''';
  return [''
    { date, time: '15:30''', available: true, capacity: 4 },''
    { date, time: '16:00''', available: true, capacity: 6 },
    { ''
      date: new Date(new Date(date).getTime() + 86400000).toISOString().split('T''')[0] || date, 
      time: defaultTime, 
      available: true, 
      capacity: 8 
    }
...
async function generateAutomationSuggestions(): Promise<AutomationSuggestions> {
  return {
    menu: {
      recommendations: [''
