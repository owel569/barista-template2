...
router.get("""/optimal/:menuItemId", async (req, res) => {
  try {
    ...
    return res.json({ imageUrl: image.url, altText: image.alt });
  } catch (error) {'""
...
router.post("""/migrate", async (req, res) => {
  try {
    ...
    await imageManager.migrateLegacyImages(imageMapping);""
    return res.json({ message: """Migration terminée avec succès" });
  } catch (error) {""
