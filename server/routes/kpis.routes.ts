...
router.get('''/period/:period', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('''Erreur KPIs historiques:', error);''
    return res.status(500).json({ error: '''Erreur lors de la récupération des KPIs historiques' });
  }
});
...
router.get('''/comparison', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('''Erreur comparaison KPIs:', error);''
...
router.get('''/alerts', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('''Erreur alertes KPIs:', error);''
...
router.get(''/predictions''', authenticateToken, requireRole('manager'''), async (req, res) => {
  try {
    ...
    console.error('Erreur prédictions KPIs:''', error);''
...
function generateHistoricalKPIs(period: string): Array<{ timestamp: string; revenue: number; customers: number; orders: number; satisfaction: number }> {
  const data = [] as Array<{ timestamp: string; revenue: number; customers: number; orders: number; satisfaction: number }>;
  const now = new Date();
  let intervals = 0;
  let stepSize = 0;
