...
const storage = {
  ...
  createInventoryOrder: async (order: InventoryOrder) => {'
  ...
  try {
    ...
    for (const item of analysisResults.criticalItems) {
      ...
      if (!approvalRequired || forceGenerate) {
        ...
        order.status = '''sent'';
        (order as InventoryOrder).sentAt = new Date().toISOString();
      }
...
async function sendOrderToSupplier(order: InventoryOrder) {''
