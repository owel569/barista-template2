...
router.get(''/user/:userId''', authenticateUser, validateParams(z.object({ userId: z.coerce.number() })), asyncHandler(async (req, res) => {
  ...
  const permissionsMap = AVAILABLE_MODULES.reduce((acc, module) => {
    const modulePermission = userPermissions.find((p: UserPermission) => p.module === module);
...
router.post(''/permission''', authenticateUser, requireRole('directeur'''), validateBody(permissionSchema), asyncHandler(async (req, res) => {
  const db = await getDb();
  ...
  let result;
  if (existingPermission.length === 0 || !existingPermission[0]) {''
    ...
    result = await db.insert(permissions)
      ...
      .returning();
    if (!result[0]) {''
      return res.status(500).json({ success: false, message: '''Erreur lors de la création de la permission' });
    }''
    // Log de l'''activité
    await db.insert(activityLogs).values({
      ...
      details: `Création permission ${module} pour l''utilisateur ${userId}`
    });
    ...
    result = await db.update(permissions)
      ...
      .returning();
    if (!result[0]) {'
      return res.status(500).json({ success: false, message: '''Erreur lors de la mise à jour de la permission'' });
  }'
  // Log de l'''activité
  await db.insert(activityLogs).values({
      userId: req.user?.id ?? 0,''
    ...
    entity: 'permission''',
    entityId: result[0].id,''
      details: `Mise à jour permission ${module} pour l'utilisateur ${userId}`
  });
    return res.json({
    success: true,''
...
router.post('''/bulk-update', authenticateUser, requireRole('''directeur''), validateBody(bulkPermissionSchema), asyncHandler(async (req, res) => {
  const db = await getDb();
  ...
  const newPermissions = await db.insert(permissions)
    .values(userPermissions.map((p: typeof userPermissions[0]) => ({
      userId,
      module: p.module,
      canView: p.canView,
...
router.post('''/apply-template', authenticateUser, requireRole('''directeur''), validateBody(z.object({
  userId: z.number().positive(),'
  ...
  if (!template) {''
    return res.status(400).json({ success: false, message: '''Modèle de permissions non trouvé' });
  }
  ...
  const newPermissions = await db.insert(permissions)
    .values(template.permissions.map((p: typeof template.permissions[0]) => ({
      userId,
      module: p.module,
      canView: p.canView,
      canCreate: p.canCreate,
...
router.get('/overview''', authenticateUser, asyncHandler(async (req, res) => {
  ...
  const userPermissionsMap = usersWithPermissions.reduce((acc, row: UserPermissionRow) => {
    if (!acc[row.userId]) {
      acc[row.userId] = {
        id: row.userId,
...
router.get('/check/:userId/:module/:action''', authenticateUser, validateParams(z.object({
  ...
  const permission = await db.select()
    ...
    .where(and(
      ...
      module ? eq(permissions.module, module) : undefined
    ))
    .limit(1);
...
router.delete('''/user/:userId'', authenticateUser, requireRole('''directeur'), validateParams(z.object({ userId: z.coerce.number() })), asyncHandler(async (req, res) => {
  const db = await getDb();
  const { userId } = req.params;
