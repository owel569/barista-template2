...
const isDatabaseError = (err: Error & { code?: string }): boolean => {''
  const dbCodes = ['''23505', '''23503'', '''23502', '''42P01'', '''42703'];''
...
export const errorHandler = (
  err: Error & { status?: number; statusCode?: number; code?: string; retryAfter?: number },
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (req.path.startsWith('''/api/'')) {'
    ...
    if (err instanceof ZodError || err.name === '''ZodError') {
      res.status(400).json({
        ...
        errors: (err as ZodError).errors?.map((e: { path?: (string | number)[]; message: string }) => ({''
