...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/admin/stats/monthly-revenue', authenticateToken, async (req, res) => {
    try {
      ...
      const revenue = stats.reduce((total, stat) => total + (stat.count * 25), 0); // Estimation
      res.json({ revenue });
    } catch (error) {
  ...
  // Routes pour la planification et le calendrier
  ...
  app.post('/api/admin/settings', authenticateToken, async (req, res) => {
    try {
      ...
      // Ici vous pouvez sauvegarder les paramètres en base de données
